...
#' @export
conversionID <- function(specie_genome = "rnorvegicus_gene_ensembl", attribute = "efg_agilent_wholegenome_4x44k_v1", externalSymbol = "rgd_symbol"){
  a<-system(paste("perl biomart-perl/scripts/converter.pl",specie_genome,attribute,externalSymbol,sep=" "),intern=TRUE,ignore.stderr = TRUE) #IF ignore.stderr is removed their is often an external 500 error
  b=a[-1]
  ...
  colnames(IDs)<-c("ensembl_gene_id","ensembl_transcript_is","ID","symbol","entrezgene_id","go_term_name","go_domain")
  return(IDs)
}
